...
def transform_data(raw_data):
    ...
    for game in raw_data:
        bookmaker_odds = next(
            (book for book in game["bookmakers"] if book["key"] == BOOKMAKER), None
        )
        ...
        home_price = next(
            (m["price"] for m in outcomes if m["name"] == game["home_team"]), None
        )
        away_price = next(
            (m["price"] for m in outcomes if m["name"] == game["away_team"]), None
        )
